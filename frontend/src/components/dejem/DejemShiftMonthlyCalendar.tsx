import type { DejemShiftCalendarDay } from "@/types/dejem";
import { dejemShiftCalendarCellClass } from "./statusStyles";
...
export function DejemShiftMonthlyCalendar({
  year,
  month,
  days,
  selected,
  ...
  const title = new Date(year, month - 1, 1).toLocaleString("pt-BR", {
    month: "long",
    year: "numeric",
  });
  ...
  return (
    <div className="rounded-xl border border-zinc-800/80 bg-black/30 p-4 shadow-inner shadow-black/30">
      <div className="mb-4 flex items-center justify-between gap-2">
        ...
        <p className="text-center text-sm font-semibold capitalize tracking-wide text-zinc-100">
          {title}
        </p>
        {hideNav ? (
          <span className="w-16" />
      ...
      <div className="mt-1 grid grid-cols-7 gap-1">
        {cells.map((c, idx) => {
          ...
          const tone = day
            ? dejemShiftCalendarCellClass(day)
            : "bg-zinc-900/30 ring-1 ring-zinc-800/40";
          ...
          return (
            <button key={c.iso} type="button" onClick={() => onSelect(c.iso)} className={baseCell}>
              <span className="text-zinc-100">{c.n}</span>
              {day && day.shift_count > 0 && (
                <span className="text-[9px] text-zinc-400">
                  {day.total_filled}/{day.total_capacity}
                </span>
              )}
            </button>
          );

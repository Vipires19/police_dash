import type { CalendarDay } from "@/types/leaves";
import { leaveStatusCellClass } from "./statusStyles";
...
interface Props {
  ...
  onPrev: () => void;
  onNext: () => void;
  /** Quando false, dias não aceitam nova solicitação (janela dia 25). */
  bookableMonth: boolean;
  /** Texto para tooltip em dias bloqueados */
  bookingBlockedTitle: string;
}
    ...
    <div className="rounded-xl border border-zinc-800/80 bg-black/30 p-4 shadow-inner shadow-black/30">
      ...
      <div className="mt-1 grid grid-cols-7 gap-1">
        {cells.map((c, idx) => {
          ...
          const tone = day ? leaveStatusCellClass(statuses) : "bg-zinc-900/30 ring-1 ring-zinc-800/40";
          const sel = selected === c.iso;
          const blocked = !bookableMonth;
          const baseCell = [
            ...
            blocked ? "cursor-not-allowed opacity-40" : "hover:brightness-110",
          ].join(" ");

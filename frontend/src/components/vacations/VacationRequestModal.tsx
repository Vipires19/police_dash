...
export function VacationRequestModal({ open, defaultStart, onClose, onSubmit }: Props) {
  ...
  const [vacationType, setVacationType] = useState<"FERIAS" | "LP">("FERIAS");
  ...
  const periodValid = totalDays === 15 || totalDays === 30;
  const periodHint =
    totalDays == null
      ? "Informe início e fim do período."
      : totalDays !== 15 && totalDays !== 30
        ? `Período de ${totalDays} dia(s): permitido apenas 15 ou 30 dias corridos.`
        : `Período válido: ${totalDays} dias corridos.`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!periodValid || !startDate || !endDate) {
      setErr("Período permitido apenas de 15 ou 30 dias corridos.");
      return;
    }
    setBusy(true);
    try {
      await onSubmit({ start_date: startDate, end_date: endDate, vacation_type: vacationType });
  ...
  return (
    <>
      <section
        ...
        role="dialog"
      >
        <section className="w-full max-w-lg rounded-xl border border-zinc-700 bg-zinc-950 p-6 shadow-2xl">
          <header className="flex items-start justify-between gap-2">
            <section>
              <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-zinc-500">Solicitação</p>
              <h3 className="mt-1 text-lg font-semibold text-zinc-50">Férias / LP</h3>
              <p className="mt-1 text-sm text-zinc-400">Período de 15 ou 30 dias corridos.</p>
            </section>
            <button
              type="button"
              onClick={onClose}
              ...
              <section className="mt-2 flex gap-2">
                <label className="flex flex-1 cursor-pointer items-center gap-2 rounded-lg border border-zinc-800 bg-black/40 px-3 py-2 text-sm">
                  <input
                    type="radio"
                    name="vt"
                    checked={vacationType === "FERIAS"}
                    onChange={() => setVacationType("FERIAS")}
                  />
                  Férias
                </label>
                <label className="flex flex-1 cursor-pointer items-center gap-2 rounded-lg border border-zinc-800 bg-black/40 px-3 py-2 text-sm">
                  <input
                    type="radio"
                    name="vt"
                    checked={vacationType === "LP"}
                    onChange={() => setVacationType("LP")}
                  />
                  LP
                </label>
              </section>
